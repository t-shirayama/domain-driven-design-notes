...
```mermaid
flowchart LR
  Presentation[Presentation / API] --> Application[Application / UseCase]
  Infrastructure[Infrastructure / EF Core / 外部 API] --> Application
  Infrastructure --> Domain[Domain / Business Rules]
  Application --> Domain

  Domain -. 参照しない .-> Infrastructure
  Domain -. 参照しない .-> Presentation
  Application -. 実装には依存しない .-> Infrastructure
```
